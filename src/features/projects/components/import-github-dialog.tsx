...
export const ImportGithubDialog = ({ open, onOpenChange }: ImportGithubDialogProps) => {
    ...
    const form = useForm({
        ...
        onSubmit: async ({ value }) => {
            try {
                const { projectId } = await ky.post("/api/github/import", { json: { url: value.url }, }).json<{ success: boolean; projectId: Id<"projects">, eventId: string; }>();
                        ...
                        {(field) => {
                            ...
                            return (
                                <Field data-invalid={isInvalid}>
                                    ...
                                    <Input id={field.name} name={field.name} value={field.state.value} onBlur={field.handleBlur} onChange={(e) => field.handleChange(e.target.value)} aria-invalid={isInvalid} placeholder="https://github.com/owner/repo" />
