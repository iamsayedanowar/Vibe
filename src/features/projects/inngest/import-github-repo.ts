...
export const importGithubRepo = inngest.createFunction(
    ...
    async ({ event, step }) => {
        ...
        const folders = tree.tree.filter((item) => item.type === "tree" && item.path).sort((a, b) => {
            ...
            for (const file of allFiles) {
                ...
                try {
                    ...
                    if (isBinary) {
                        ...
                        const { storageId } = await ky.post(uploadUrl, { headers: { "Content-Type": "application/octet-stream" }, body: buffer }).json<{ storageId: Id<"_storage">; }>();
